...
// Copyright (c) 2017-2018 Criptoreal Developers
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params) {
    ...
    if (BlockLastSolved == NULL || BlockLastSolved->nHeight < Params().SwitchCriptoRealblock() + PastBlocksMin) {
        return UintToArith256(params.powLimit).GetCompact();
    }

    for (unsigned int i = 1; BlockReading && BlockReading->nHeight >= Params().SwitchCriptoRealblock(); i++) {
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if(pindexLast->nHeight+1 >= Params().SwitchCriptoRealblock())
    {
        // DGW v3
    ...
    // Criptoreal: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    bnOld = bnNew;
    // Criptoreal: intermediate uint256 can overflow by 1 bit
