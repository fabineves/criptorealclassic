...
#if defined(NDEBUG)
# error "Criptoreal cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Criptoreal Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s CRS additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetPoWHash(nHeight >= Params().SwitchCriptoRealblock()), block.nBits, consensusParams))
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (halvings >= 64)
      ...
      int CRSfund = nHeight;
    // Blocks 1-8 = 5 mil CRS for Development Fund.
    // Read More https://criptoreal.org/CRSfund
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(nHeight >= Params().SwitchCriptoRealblock()), block.nBits, consensusParams))
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Criptoreal: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
