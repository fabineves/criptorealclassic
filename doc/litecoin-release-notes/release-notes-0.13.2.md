Criptoreal Core version 0.13.2 is now available from:

  <https://download.criptoreal.org/criptoreal-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a criptoreal
...
ECDSA signatures inside Criptoreal transactions now use validation using
...
Previous versions of Criptoreal Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Criptoreal Core was to raise the default minimum
relay fee.

Criptoreal Core 0.13.2 will have a strict maximum size on the mempool. The
...
Criptoreal Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running criptoreald without having to do any manual configuration.
...
Criptoreal Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Criptoreal Core will relay transactions with
...
entirely in Criptoreal Core 0.13, and it is at this time undecided whether the
...
manual configuration. Criptoreal Core will also use Tor automatically to connect
...
Criptoreald can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Criptoreal
Core to use floating fees. By default, Criptoreal Core will use floating
fees.
...
(default: `0.0002` CRS/kB).

At all times, Criptoreal Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CRS.
Furthermore, Criptoreal Core will never create transactions paying less than
...
`criptoreal.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /criptorealrpc>
        ...
        # optional bypass criptoreald rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< criptorealrpc:<password>
    </Location>
...
Restarting criptoreald will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in criptoreal.conf
...
criptoreal-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/criptoreal-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Criptoreal Core code base has
...
- `criptoreal-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `criptoreal-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Criptoreal scripting system that in combination with
...
Criptoreal transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Criptoreal Core.
...
The two phases are distinct in the Criptoreal-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Criptoreal wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Criptoreal.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Criptoreal block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of criptoreals they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of criptoreals being spent is exactly
...
- **Increased security for multisig:** Criptoreal addresses (both P2PKH addresses
  ...
  believed to be provided by Criptoreal's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Criptoreal's current ledger (called the UTXO set),
  ...
  note that Criptoreal Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Criptoreal users to individually opt-in to almost any change in the Criptoreal
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Criptoreal Core contributors that may
...
segwit's start date of 1 January 2018 miners can update the Criptoreal
client to Criptoreal Core 0.13.2 to signal for segwit support. When a
...
Since Criptoreal Core 0.10.0, nodes have defaulted to only relaying and
...
- `criptoreal-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `criptoreal-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
